-- Hutao [forsaken] V1.0.9 (Modified with FINAL AUTO BLOCK HDT)
...
combatS.hdtSpeed = 26          -- Dspeed from FINAL AUTO BLOCK
combatS.hdtDelay = 0           -- Ddelay from FINAL AUTO BLOCK
combatS.hdtDuration = 1.0      -- drag duration
combatS.hdtMissChance = 0
combatS.hdtMoveSpeed = 26      -- kept for fallback
...
-- Block anim IDs for HDT detection (Hutao's original BLOCK_ANIMS)
local BLOCK_ANIMS = {
...
-- =========================================================================
-- HDT (Hitbox Dragging Tech) - FINAL AUTO BLOCK VERSION
-- Uses BodyVelocity for smooth dragging, preserves Hutao's BLOCK_ANIMS
-- =========================================================================
...
combatGetKillerHRP = function(killerModel)
    ...
    if killerModel:FindFirstChild("HumanoidRootPart") then
        return killerModel.HumanoidRootPart
    end
...
local combatHDTBodyVelocity = nil
local combatHDTSavedWalk = nil
local combatHDTSavedJump = nil
local combatHDTSavedPlatformStand = nil
...
local function combatHDTBeginDrag(killerModel, blockTrack)
    pcall(function()
        ...
        -- Save original locomotion state
        combatHDTSavedWalk = hum.WalkSpeed
        combatHDTSavedJump = hum.JumpPower
        combatHDTSavedPlatformStand = hum.PlatformStand
        ...
        -- Create BodyVelocity to push HRP toward killer (like FINAL AUTO BLOCK)
        if combatHDTBodyVelocity then combatHDTBodyVelocity:Destroy() end
        combatHDTBodyVelocity = Instance.new("BodyVelocity")
        combatHDTBodyVelocity.MaxForce = Vector3.new(1e5, 0, 1e5)
        combatHDTBodyVelocity.Velocity = Vector3.new(0, 0, 0)
        combatHDTBodyVelocity.Parent = hrp
        ...
        local function stopDrag()
            ...
            if combatHDTBodyVelocity and combatHDTBodyVelocity.Parent then
                pcall(function() combatHDTBodyVelocity:Destroy() end)
                combatHDTBodyVelocity = nil
            end
            
            if hum and hum.Parent then
                hum.WalkSpeed = combatHDTSavedWalk or 16
                hum.JumpPower = combatHDTSavedJump or 50
                hum.PlatformStand = combatHDTSavedPlatformStand or false
            end
            
            combatHDTSavedWalk = nil
            combatHDTSavedJump = nil
            combatHDTSavedPlatformStand = nil
        end
        ...
        -- Timer to auto-stop
        task.delay(combatS.hdtDuration, stopDrag)

        -- Apply delay before starting drag (Ddelay from FINAL AUTO BLOCK)
        if combatS.hdtDelay and combatS.hdtDelay > 0 then
            task.wait(combatS.hdtDelay)
            if stopped then return end
            -- refresh references after delay
            char = lp.Character
            hrp = char and char:FindFirstChild("HumanoidRootPart")
            hum = char and char:FindFirstChildOfClass("Humanoid")
            targetHRP = killerModel and combatGetKillerHRP(killerModel)
            if not hrp or not hum or not targetHRP then stopDrag(); return end
            if combatHDTBodyVelocity and combatHDTBodyVelocity.Parent then
                combatHDTBodyVelocity.Parent = hrp
            end
        end

        -- Drag loop (same as FINAL AUTO BLOCK's beginDragIntoKiller)
        while not stopped and combatHDTDragging do
            -- Refresh references
            char = lp.Character
            hrp = char and char:FindFirstChild("HumanoidRootPart")
            hum = char and char:FindFirstChildOfClass("Humanoid")
            targetHRP = killerModel and killerModel.Parent and combatGetKillerHRP(killerModel)
            
            if not hrp or not hum or not targetHRP then
                stopDrag()
                break
            end
            ...
            if combatHDTBodyVelocity and combatHDTBodyVelocity.Parent then
                if horiz.Magnitude > 0.01 then
                    ...
                    combatHDTBodyVelocity.Velocity = Vector3.new(dir.X * combatS.hdtSpeed, combatHDTBodyVelocity.Velocity.Y, dir.Z * combatS.hdtSpeed)
                else
                    combatHDTBodyVelocity.Velocity = Vector3.new(0, combatHDTBodyVelocity.Velocity.Y, 0)
                end
            end
            
            -- Stop condition: very close to killer
            local stopDist = 2.0
            if toTarget.Magnitude <= stopDist then
                stopDrag()
                break
            end
            
            -- Also stop if killer died or left
...
-- Uses Hutao's original BLOCK_ANIMS IDs
...
sec_017:Slider({ Title = "HDT Speed", Flag = "combatHDTSpeed", Value = {Min=1,Max=100,Default=combatS.hdtSpeed}, Step = 1, Callback=function(v) combatS.hdtSpeed=v end
})

sec_017:Slider({ Title = "HDT Delay (s)", Flag = "combatHDTDelay", Value = {Min=0,Max=1.0,Default=combatS.hdtDelay}, Step = 0.05, Callback=function(v) combatS.hdtDelay=v end
})
