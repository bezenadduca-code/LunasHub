...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabElliot  = win:CreateTab("Elliot",    "pizza")
local tabChance  = win:CreateTab("Chance",    "crosshair")
local tabTwoTime = win:CreateTab("TwoTime",   "knife")
local tabSurSen  = win:CreateTab("Guest1337", "shield")

tabElliot:CreateSection("Elliot Aimbot")

-- Elliot Aimbot
do
    local tabSurSen = tabElliot
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabDusekkar = win:CreateTab("Dusekkar", "zap")

do
    tabDusekkar:CreateSection("PlasmaBeam Silent Aim")

    tabDusekkar:CreateParagraph({
        Title   = "How it works",
    ...
    tabDusekkar:CreateToggle({
        Name = "Enable PlasmaBeam Aim", CurrentValue = dusk_enabled, Flag = "duskEnabled",
        Callback = function(on) dusk_enabled = on; cfg.set("duskEnabled", on) end
    })
    tabDusekkar:CreateSlider({
        Name = "Prediction (s)", Range = { 0.0, 0.5 }, Increment = 0.01,
        CurrentValue = dusk_prediction, Flag = "duskPrediction",
        Callback = function(v) dusk_prediction = v; cfg.set("duskPrediction", v) end
    })
    tabDusekkar:CreateSlider({
        Name = "Aim Height Offset", Range = { -3.0, 3.0 }, Increment = 0.1,
        CurrentValue = dusk_aimOffset, Flag = "duskAimOffset",
        Callback = function(v) dusk_aimOffset = v; cfg.set("duskAimOffset", v) end
    })
    tabDusekkar:CreateSection("Control")
    tabDusekkar:CreateButton({
        Name = "Unload Dusekkar Hook", Callback = function()
            dusk_enabled = false
            if dusk_oldNC then
                pcall(function()
