...
local function combatHDTBeginDrag(killerModel, blockTrack)
    pcall(function()
        ...
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        local killerRoot = killerModel and (killerModel:FindFirstChild("HumanoidRootPart") or killerModel.PrimaryPart)

        if not hum or not hrp or not killerRoot then
        ...
        local function stopHDT()
            if not combatHDTDragging then return end
            combatHDTDragging = false

            if combatHDTConn then
                combatHDTConn:Disconnect()
                combatHDTConn = nil
            end
        ...
        -- Continuous override loop every frame
        combatHDTConn = svc.Run.RenderStepped:Connect(function()
            if not combatHDTDragging or not hum or not hrp or not killerRoot.Parent then
                stopHDT()
                return
            end
            ...
            if dist > 1 then
                ...
                -- Direct velocity push so sprint module cannot fight back
                hrp.AssemblyLinearVelocity = Vector3.new(
                    unit.X * combatS.hdtMoveSpeed * 2.2,
                    hrp.AssemblyLinearVelocity.Y,
                    unit.Z * combatS.hdtMoveSpeed * 2.2
                )
        ...
        -- Cleanup on block end
        if blockTrack then
            blockTrack.Stopped:Connect(stopHDT)
        end

        -- Safety timeout (max 2.5 seconds)
        task.delay(2.5, stopHDT)
    end)
end
