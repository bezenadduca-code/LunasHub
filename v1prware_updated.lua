...
do
    ...
    local function getKillerAbility()
        -- Check which ability YOUR killer has: Slash or Punch
        local char = lp.Character; if not char then return nil end
        
        -- Check for Slash ability
        if char:FindFirstChild("Slash") or char:FindFirstChildOfClass("Tool") and char:FindFirstChildOfClass("Tool").Name == "Slash" then
            return "Slash"
        end
        
        -- Check for Punch ability
        if char:FindFirstChild("Punch") or char:FindFirstChildOfClass("Tool") and char:FindFirstChildOfClass("Tool").Name == "Punch" then
            return "Punch"
        end
            ...
            if targetHRP then
                ...
                if distance <= 13 then
                    ...
                    if currentTime - lastAbilityTime >= abilityFireCooldown then
                        local killerAbility = getKillerAbility()
                        if killerAbility then
                            fireAbility(killerAbility)
