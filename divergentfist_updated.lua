...
local uppercutRemote = getRemote("Knit", "Knit", "Services", "ItadoriService", "RE", "Activated")
if not uppercutRemote then
    warn("[DivergentFist] Uppercut remote not found")
end

local chaseRemote = getRemote("Knit", "Knit", "Services", "ItadoriService", "RE", "Chase")
if not chaseRemote then
    warn("[DivergentFist] Chase remote not found")
end
...
--  UPPERCUT 360 DASH
-- ──────────────────────────────────────────────
local uppercutDashEnabled = false

local function perform360OrbitDash(targetRoot)
    local hrp = getHRP()
    if not hrp or not targetRoot or not targetRoot.Parent then return end
    ...
    local orbitDuration = 0.55

    createTrail(hrp)
    local dashTrack = playDashAnimation(dashDirection, orbitDuration)

    hrp.AssemblyLinearVelocity = Vector3.zero

    local entryAngle = math.atan2(
        hrp.Position.X - targetRoot.Position.X,
        hrp.Position.Z - targetRoot.Position.Z
    )
    ...
    connection = RunService.Heartbeat:Connect(function(dt)
        if not targetRoot or not targetRoot.Parent or elapsed >= orbitDuration then
            connection:Disconnect()
            if dashTrack and dashTrack.IsPlaying then
                dashTrack:Stop(0.1)
            end
        ...
        elapsed = elapsed + dt
        local t = math.clamp(elapsed / orbitDuration, 0, 1)
        local st = t * t * (3 - 2 * t)

        local currentTargetPos = targetRoot.Position
        local angle = entryAngle + (spinSign * st * math.pi * 2)
        local orbitPos = currentTargetPos + Vector3.new(
            math.sin(angle) * CONFIG.LoopRadius,
            0,
            math.cos(angle) * CONFIG.LoopRadius
        )

        hrp.CFrame = CFrame.lookAt(
            Vector3.new(orbitPos.X, hrp.Position.Y, orbitPos.Z),
            currentTargetPos
        )
    end)

    task.wait(orbitDuration)
end

local uppercutOldNamecall
uppercutOldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    if getnamecallmethod() ~= "FireServer" then
        return uppercutOldNamecall(self, ...)
    end

    if self ~= uppercutRemote then
        return uppercutOldNamecall(self, ...)
    end
    ...
    local result = uppercutOldNamecall(self, ...)

    if uppercutDashEnabled then
        local target = findNearestTarget()
        local targetRoot = target and target:FindFirstChild("HumanoidRootPart")

        if targetRoot and targetRoot.Parent then
            task.spawn(function()
                -- Wait for uppercut to finish
                task.wait(0.9)

                if not targetRoot.Parent then return end

                -- Fire Chase so server knows dash is happening
                pcall(function()
                    chaseRemote:FireServer(false)
                end)

                -- Physically do the 360 orbit
                perform360OrbitDash(targetRoot)
            end)
        end
    end
...
-- ── ITADORI TAB ──────────────────────────────────
local ItadoriTab = Window:Tab({ Title = "Itadori", Icon = "zap" })

ItadoriTab:Toggle({
    Title    = "Uppercut 360 Dash",
    Value    = uppercutDashEnabled,
    Callback = function(value)
        uppercutDashEnabled = value
    end,
})
