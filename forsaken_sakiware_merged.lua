...
------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: FORSAKEN ESP (Document / Ring)
------------------------------------------------------------------------
------------------------------------------------------------------------
local secForsakenESP = tabVisual:Section({ Title = "Forsaken ESP", Opened = true })

local fskEnabled   = false
local fskTracked   = {} -- table of { highlight, billboard } per obj

local function fskRemoveAll()
    for obj, t in pairs(fskTracked) do
        pcall(function()
...
local function fskRemoveOne(obj)
    local t = fskTracked[obj]
...
local function fskMake(obj)
    if not fskEnabled then return end
    if fskTracked[obj] then return end
    ...
    if txt:find("collect") or txt:find("document") or txt:find("folder") then
        itemType = "DOCUMENT"
    elseif txt:find("ring") then
        itemType = "RING"
    end

    if not itemType then return end

    local fillColor    = itemType == "RING" and Color3.fromRGB(255, 215, 0) or Color3.fromRGB(255, 255, 0)
    local outlineColor = itemType == "RING" and Color3.fromRGB(255, 165, 0) or Color3.fromRGB(255, 255, 255)

    local h = Instance.new("Highlight")
    h.Name                = "ForsakenESP_" .. itemType
    ...
    bill.Name        = itemType == "RING" and "RING" or "DOCUMENT"
    ...
    fskTracked[obj] = { highlight = h, billboard = bill }

    -- auto-clean when item is removed
    ...
    print("[ForsakenESP] " .. itemType .. " found: " .. obj:GetFullName())
end

local function fskScan()
    for _, v in ipairs(svc.WS:GetDescendants()) do
        pcall(function() fskMake(v) end)
    end
end
