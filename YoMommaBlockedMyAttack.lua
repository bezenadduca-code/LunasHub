...
-- Additional autoblock method toggles
combatS.autoBlockOn      = false  -- Hitbox-based
combatS.autoBlockAudioOn = false  -- Sound-based
combatS.autoBlockAnimOn  = false  -- Animation-based
...
local function combatAttemptSoundBlock(sound, preId)
    if not combatS.autoBlockAudioOn then return end
    ...
    -- Must be a killer, not a survivor
...
local function combatSetupSoundHooks()
    ...
    for _, d in ipairs(kf:GetDescendants()) do
        if d:IsA("Sound") then pcall(combatHookSound, d) end
    end
...
local function combatSetupAnimBlock()
    if combatAnimBlockConn then combatAnimBlockConn:Disconnect(); combatAnimBlockConn = nil end
    if not combatS.autoBlockAnimOn then return end

    combatAnimBlockConn = svc.Run.RenderStepped:Connect(function()
        if not combatS.autoBlockAnimOn then return end
        local myRoot = combatCachedHRP; if not myRoot then return end
        local kf = combatGetKillersFolder(); if not kf then return end
...
local function combatTryBlockFromHitbox(hb)
    ...
    -- Ownership check: read the Creator tag Forsaken stamps on every hitbox.
    -- The game stores it as either an attribute OR a StringValue child named "Creator".
    local kf = combatGetKillersFolder()
    if not kf then return end

    local creatorName = nil
    -- Try attribute first (fastest, no instance traversal)
    creatorName = hb:GetAttribute("Creator")
        or hb:GetAttribute("creator")
        or hb:GetAttribute("Owner")
        or hb:GetAttribute("owner")
    -- Fall back to StringValue child
    ...
    -- If we couldn't read a creator at all, fall back to checking
    -- whether the hitbox name matches any killer model name exactly
    ...
    -- Still nothing — this hitbox did not come from a killer, ignore it
...
sec_015:Toggle({ Title = "Auto Block (SOUNDS)", Flag = "combatAutoBlockSounds", Default = combatS.autoBlockAudioOn, Callback=function(on)
        combatS.autoBlockAudioOn = on
        if on then combatSetupSoundHooks() end
    end, Type = "Checkbox"})

sec_015:Toggle({ Title = "Auto Block (ANIMATION)", Flag = "combatAutoBlockAnim", Default = false, Callback=function(on)
        combatS.autoBlockAnimOn = on
